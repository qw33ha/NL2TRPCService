...
class NL2ServiceWorkflow:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        graph.add_node("select_examples", self._select_examples_node)
        ...
        graph.add_node("monitor_github_actions", self.github_actions.monitor)
        graph.add_node("prepare_delivery_report", self.github_actions.prepare_report)
        ...
        graph.add_conditional_edges(
            ...
            self._route_start,
            {
                ...
                "monitor_github_actions": "monitor_github_actions",
            },
        )
        ...
        graph.add_edge("deliver_to_github", "monitor_github_actions")
        graph.add_conditional_edges(
            "monitor_github_actions",
            self.github_actions.route,
            {
                "repair_build_errors": "repair_build_errors",
    ...
    def _route_start(self, state: WorkflowState) -> str:
        ...
        if interaction_type == "github_actions_poll" or state.get("status") == "awaiting_github_actions":
            return "monitor_github_actions"
    ...
    def _route_after_verify_build(self, state: WorkflowState) -> str:
        if state.get("status") == "verified":
            if state.get("target_phase") == "deliver":
                if state.get("github_delivery", {}).get("owner") and state.get("github_delivery", {}).get("repo"):
